...
async function main() {
  const contractAddress = process.env.SIGNALFORCE_LEDGER_ADDRESS || process.env.RISK_LEDGER_ADDRESS;
  const postId = process.env.POST_ID;
  const postHash = process.env.POST_HASH;

  if (!contractAddress || !postId || !postHash) {
    throw new Error(
      "Set SIGNALFORCE_LEDGER_ADDRESS (or RISK_LEDGER_ADDRESS), POST_ID and POST_HASH env vars before running."
    );
  }
